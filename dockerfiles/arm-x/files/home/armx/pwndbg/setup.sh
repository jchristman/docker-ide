...
install_apk() {
    if cat /etc/apk/repositories | grep -q "edge"; then
        sudo apk update || true
        sudo apk add gdb python2 python2-dev python3 python3-dev py2-setuptools py3-pip libc-dev glib-dev
        sudo easy_install-2.7 pip
    else
        echo "You must be on edge repositories for Alpine Linux because package names have changed."
        echo "The following command *might* do the trick:"
        echo "sed -i -e \'s/v[[:digit:]]\..*\//edge\//g\' /etc/apk/repositories"
        exit 1
    fi
}
